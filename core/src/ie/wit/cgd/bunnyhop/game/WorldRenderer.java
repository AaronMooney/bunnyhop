...
public class WorldRenderer implements Disposable {
	...
	private void renderGuiExtraLive(SpriteBatch batch) {
		float x = cameraGUI.viewportWidth - 50 - Constants.LIVES_MAX * 50;
		float y = -15;
		for (int i = 0; i < Constants.LIVES_MAX; i++) {
	...
	private void renderGuiGameOverMessage(SpriteBatch batch) {
		...
		} else if (worldController.isGameWon()) {
			BitmapFont fontGameOver = Assets.instance.fonts.defaultBig;
			fontGameOver.setColor(1, 0.75f, 0.25f, 1);
	...
	private void renderGuiTimeLeft(SpriteBatch batch) {
		...
		float y = 30;
		float timeLeft = worldController.level.bunnyHead.timeLeft;

		if (timeLeft > 0) {
			if (((int) (timeLeft * 5) % 2) != 0)
				Assets.instance.fonts.defaultBig.draw(batch, "Time Left : " + (int) timeLeft, x, y);
			else
				Assets.instance.fonts.defaultBig.draw(batch, "Time Left : " + (int) timeLeft, x, y);
		}
	}
